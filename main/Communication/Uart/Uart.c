/**
 * @file app_uart.c
 * @brief UART0 驱动封装：固定大小队列版本，避免频繁 malloc/free
 */
 ...
 */
#define APP_UART_RX_RING_BUF_SIZE  2048
 ...
 * UART 驱动事件队列长度。
 */
#define APP_UART_EVENT_QUEUE_LEN   20
 ...
 * 接收任务配置。
*/
#define APP_UART_TASK_STACK_SIZE   LG_STACK_UART0_RX

#define APP_UART_TASK_PRIORITY     LG_PRIO_UART0_RX
...
static esp_err_t app_UART0_init_internal(void);
...
static esp_err_t app_UART0_init_internal(void)
{
    ...
    ret = uart_driver_install(
        APP_UART_PORT,
        APP_UART_RX_RING_BUF_SIZE,
        APP_UART_TX_RING_BUF_SIZE,
        APP_UART_EVENT_QUEUE_LEN,
        &s_uart_event_queue,
        0
    );
...
{
    esp_err_t ret = app_UART0_init_internal();
