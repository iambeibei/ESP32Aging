...
/* xmodem crc16 table */
static const unsigned short xm_crc_tbl[256] = {
...
static const unsigned int CRC32_MPEG2_Table[] =
    {
        0x00000000,
        0x04c11db7,
        0x09823b6e,
...
uint16_t Modbus_Crc16Cal(const uint8_t *ptr, int len)
{
    ...
    // initial value
    register uint8_t *ptr_pkt_hdr = (uint8_t *)ptr;
    c2 = len;
    for (c1 = 0; c1 < c2; c1++)
    {
...
{
    register uint16_t crc = 0x0000;

    while (len--)
    {
        crc = (crc << 8) ^ xm_crc_tbl[(crc >> 8 ^ *ptr++) & 0xFF];
    }
...
uint32_t calcu_crc32(uint32_t icrc, const uint8_t *data, int length)
{
    ...
    while (length) {
        ...
        if (length >= 4) {
            ...
            for (uint8_t i = (length * 8); i > 0; i -= 8) { 
                word |= data[i] << i;
            }
            length = 0;
        }
...
{
    uint32_t XOROT = 0x00000000;
    uint32_t uCRCValue = icrc;

    // #if USED_CRC32_FIND_TABLE
    /* calculate CRC crc32-mpeg2*/
    for (int i = 0; i < len; i++)
    {
        uCRCValue = CRC32_MPEG2_Table[((uCRCValue >> 24) ^ data[i]) & 0xFF] ^ (uCRCValue << 8);
    }

    // #else
    // while (len > 0)
    //{
    //     uCRCValue ^= (unsigned int)(*pData++) << 24;
    //     for (uint8_t i = 0; i < 8; i++)
    //     {
    //         if (uCRCValue & 0x80000000)
    //             uCRCValue = (uCRCValue << 1) ^ 0x04C11DB7;
    //         else
    //             uCRCValue <<= 1;
    //     }
    // }
